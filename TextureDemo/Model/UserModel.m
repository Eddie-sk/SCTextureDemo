...
- (void)fetchCompleteUserData
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *urlString     = [NSString stringWithFormat:@"https://api.500px.com/v1/users/show?id=%@&consumer_key=Fi13GVb8g53sGvHICzlram7QkKOlSDmAmp9s9aqC", self->_userID];
        ...
        NSURLSessionDataTask *task = [session dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            if (data) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    weakSelf->_fullUserInfoFetchDone = YES;
                    
                    if (weakSelf->_fullUserInfoCompletionBlock) {
                        weakSelf->_fullUserInfoCompletionBlock(weakSelf);
                        ...
                        weakSelf->_fullUserInfoCompletionBlock = nil;
                    }
                });
            }
        }];
