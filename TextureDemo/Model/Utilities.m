...
- (CGSize)boundingSizeWithAttribuiteDic:(NSDictionary *)attribuite maxWidth:(CGFloat)width maxNumOfLines:(NSInteger)maxLine  {
    ...
    CGFloat fontSize = 0;
    NSValue *value = [attribuite valueForKey:NSFontAttributeName];
    if ([value isKindOfClass:UIFont.class]) {
        UIFont *font = (UIFont *)value;
        fontSize = font.lineHeight;
    }
    CGFloat maxHeight = maxLine * fontSize;
    CGSize boundingSize = [self boundingRectWithSize:CGSizeMake(width, maxHeight + 1.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : value} context:nil].size;
    return boundingSize;
}
